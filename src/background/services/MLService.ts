...
export class MLService {
  ...
  /**
   * Get difficult sequences from ML Worker
   */
  async getDifficultSequences(): Promise<any> {
    try {
      return await this.sendToWorker({
        type: 'getDifficultSequences'
      });
    } catch (error) {
      console.error('[MLService] Failed to get difficult sequences:', error);
      throw error;
    }
  }

  /**
   * Get difficult patterns from ML Worker
   */
  async getDifficultPatterns(): Promise<any> {
    try {
      return await this.sendToWorker({
        type: 'getDifficultPatterns'
      });
    } catch (error) {
      console.error('[MLService] Failed to get difficult patterns:', error);
      throw error;
    }
  }

  /**
   * Apply LLM insights to improve ML Worker predictions
   */
  async applyLLMInsights(insights: Array<{pattern: string, intent: string, confidence: number}>): Promise<void> {
    try {
      await this.sendToWorker({
        type: 'applyLLMInsights',
        data: { insights }
      });
      console.log(`[MLService] Applied ${insights.length} LLM insights to ML Worker`);
    } catch (error) {
      console.error('[MLService] Failed to apply LLM insights:', error);
      throw error;
    }
  }

  /**
   * Process synthetic training data generated by LLM
   */
  async processSyntheticTrainingData(syntheticData: any[]): Promise<any> {
    try {
      const result = await this.sendToWorker({
        type: 'processSyntheticTrainingData',
        data: { syntheticData }
      });
      console.log(`[MLService] Processed ${syntheticData.length} synthetic training samples`);
      return result;
    } catch (error) {
      console.error('[MLService] Failed to process synthetic training data:', error);
      throw error;
    }
  }

  /**
   * Perform few-shot learning with high-confidence synthetic data
   */
  async fewShotLearning(syntheticData: any[], learningRate: number = 0.01): Promise<any> {
    try {
      const result = await this.sendToWorker({
        type: 'fewShotLearning',
        data: { syntheticData, learningRate }
      });
      console.log(`[MLService] Few-shot learning completed with ${syntheticData.length} samples`);
      return result;
    } catch (error) {
      console.error('[MLService] Few-shot learning failed:', error);
      throw error;
    }
  }

  /**
   * Reset difficulty tracking in ML Worker
   */
  async resetDifficultyTracking(): Promise<void> {
    try {
      await this.sendToWorker({
        type: 'resetDifficultyTracking'
      });
      console.log('[MLService] Reset difficulty tracking in ML Worker');
    } catch (error) {
      console.error('[MLService] Failed to reset difficulty tracking:', error);
      throw error;
    }
  }
